...
void display() {
	...
	if (enable_lighting) {
		...
		for (int i = 0; i < numused; ++i){
			if (dynamiclight[i] >= 0 && lightposn[4*i+3] != 0.0f) { // only move point light sources
				...
				transformvec(dynamicposn, &lightransf[4*i]);
			} else{
	...
	for (int i = 0; i < numtableobjects; ++i){
		...
		mat4 transform = mv * obj->transform;
		...
		vec4 pos = mv * vec4(tableobjects[i].pos.x, tableobjects[i].pos.y, tableobjects[i].pos.z, 1.0f);
